"""
This module is for producing detailed results with model >= v3.7
with sex and birth episodes flag in inpatients.

Assumes you have already authenticated via az. Outputs into a `data/` folder the detailed
aggregations of IP, OP, and AAE model results in CSV and Parquet formats.

Also assumes the scenario has already been run with full_model_results = True. Run the
run-scenario-with-full-results notebook if you need to do this.
"""
